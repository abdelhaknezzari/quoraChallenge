...
# HOW
 ...
 train_word_vec_glove <- reactive({
   input$OPTIONS_WORD_VEC_GLOVE
   input$GloveVectorsDimention
   input$GloveWindowLength
   input$n_iter
   input$x_max
   isolate({
         switch(input$OPTIONS_WORD_VEC_GLOVE,
            '1'= {
              load_word_vectors()
            },
            '2' = {
              
              list(texts_ = rbind(data_set()[,"question1"],data_set()[,"question2"]),
                   skip_grams_window = input$GloveWindowLength , x_max = input$x_max , n_iter = input$n_iter , word_vectors_size = input$GloveVectorsDimention ) %>% 
                tokenize_text_() %>% 
                glove_create_vocab() %>% 
 ...
 output$SIMILARITY_MATRIX_PLOT <- renderPlot({
   ...
   data_set( )[input$dataset1_rows_selected,] %>% plot_conv_matrix_q1q2(word_vectors = train_word_vec_glove() )
   
 })
 
 
 
get_similar_words_reactive <- reactive({
  input$select_WORD
  input$numberOfWordNeigbours
  isolate({
    train_word_vec_glove() %>%
       get_similar_words(input$select_WORD,.,input$numberOfWordNeigbours) %>% 
      train_word_vec_glove()[., , drop = FALSE]  

  })
})
 
output$WORD_VEC_D2_TSNE <- renderPlot({
   req(input$select_WORD)
   get_similar_words_reactive() %>% tsne_word_of_interest()

})

output$QUESTION_WORD_VEC_PLOT <- renderPlot({
  req(input$dataset1_rows_selected)
  req(input$OPTIONS_QUESTIONS_WORD_VEC)
  
  train_word_vec_glove() %>% plot_word_vector_questions_vectors(data_set( )[input$dataset1_rows_selected,],.,input$OPTIONS_QUESTIONS_WORD_VEC)
})
...
# SHOW
 ...
 train_model_test_metrics <- reactive({
  ...
  output$dataset1 <- DT::renderDataTable({
    data_set( )%>% DT::datatable( options = list(pageLength = 5,scrollX = T),selection = 'single')
  })
  
   
  output$dataset_features <- DT::renderDataTable({
    features_data( )%>% DT::datatable( options = list(pageLength = 5,scrollX = T),selection = 'single')
  })
  ...
  # }) 
   ...
   output$FEAT1_TABLE <- DT::renderDataTable({
       req(input$dataset1_rows_selected)
       data_set( )[input$dataset1_rows_selected,] %>% get_features1() %>% DT::datatable( options = list(searching = FALSE,scrollX = T))
     })
  
    output$FEAT2_TABLE <- DT::renderDataTable({
       req(input$dataset1_rows_selected)
       data_set( )[input$dataset1_rows_selected,] %>% get_features2() %>% DT::datatable( options = list(searching = FALSE,scrollX = T))
     })
