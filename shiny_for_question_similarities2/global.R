...
load_word_vectors_glove <- function()
{
  ...
  wv <- word_vectors_glove
  rm(word_vectors_glove)
  wv
}
...
get_similar_words <- function( word,word_vectors ,nighbours_nr) {
  word_vec = word_vectors[word, , drop = FALSE]                           
  cos_sim = sim2(x = word_vectors, y = word_vec, method = "cosine", norm = "l2")
  word_nighbours_ <- head(sort(cos_sim[,1], decreasing = TRUE), nighbours_nr)
  word_nighbours_ %>% names()
}
get_some_word_of_interst <- function(word_vectors,word_of_interst,nighbours_nr) {
  # word_of_interst <- c("happy","child","health")
  word_nighbours <- word_of_interst %>% lapply(FUN = get_similar_words,word_vectors,nighbours_nr)
...
tsne_word_of_interest <- function(words_of_interst1) {
  set.seed(9)  
  tsne_model_1 = words_of_interst1 %>% as.matrix( ) %>% Rtsne( check_duplicates=FALSE, pca=TRUE, perplexity=3,theta=0.5, dims=2)
  ...
  ggplot(d_tsne_1, aes(x= X1, y = X2)) + 
    ...
    geom_label_repel(aes(label = term),segment.color = "grey50") +
    theme_classic()
}

  
# get_word_vectors() %>%
#   get_similar_words("health",.,100) %>% 
#   get_word_vectors()[., , drop = FALSE]  %>%
#   tsne_word_of_interest()


# get_some_word_of_interst(get_word_vectors(),"health" %>%   get_similar_words(get_word_vectors() ,10) ,5)  %>%
#   get_word_vectors()[., , drop = FALSE]  %>%
#   tsne_word_of_interest()


# word_vec %>% get_some_word_of_interst() %>% word_vec[., , drop = FALSE] %>% tsne_word_of_interest()
# 
# word_vec %>% get_similar_words("family",.)
# 
# word_vec %>% get_some_word_of_interst()
...
get_free_ram <- function(){
   ...
   used_memory <- pryr::mem_used()/100000000 %>% round(2)
